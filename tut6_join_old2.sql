...
/* Ex1. Find the title and artist who recorded the song 'Alison'. */
SELECT title, artist
FROM album a
JOIN track t
ON a.asin = t.album
...
/* Ex2. Which artist recorded the song 'Exodus'? */
SELECT artist
FROM album a
JOIN track t
ON a.asin = t.album
...
/* Ex3. Show the song for each track on the album with title'Blur'. */
SELECT song
FROM track t
JOIN album a
ON t.album = a.asin
...
/* Ex5. For each album show the title and the total number of tracks containing 
...
/* Ex6. A "title track" is where the song is the same as the title. 
...
/* Ex7. An "eponymous" album is one where the title is the same as the artist 
...
/* Ex8. Find the songs that appear on more than 2 albums. 
...
/* Ex9. A "good value" album is one where the price per track is less than 50 pence. 
...
/* Ex10. Wagner's Ring cycle has an imposing 173 tracks, 
Bing Crosby clocks up 101 tracks.
